...
class DTOAnnotator : Annotator {
    ...
    private class DTOAnnotatorVisitor(private val holder: AnnotationHolder) : DTOVisitor() {
        ...
        override fun visitDto(o: DTODto) {
            ...
            val inputModifiers = currentModifiers.zip(o.modifiers).filter { it.second.isInputStrategy }
        ...
        override fun visitDtoName(o: DTODtoName) {
            ...
            if (o.file.dtos.count { it == o.value } > 1) {
                o.error("Duplicated dto `${o.value}`")
            }
        }
        ...
        fun visitAnnotationName(qualifiedName: DTOQualifiedName) {
            ...
            if (simpleName == "TNullable" && annotationName != JimmerAnnotations.TNullable) {
        ...
        override fun visitUserProp(o: DTOUserProp) {
            ...
            o.containingLClass?.findProperty(o.name.value)?.let {
                o.name.error("It is prohibited for user-prop and entity prop to have the same name")
                return
            }
        ...
        private fun visitUserPropType(o: DTOTypeRef) {
            ...
            if (type in dtoFile.dtos) {
                o.type.error("It is not allowed to use a DTO, as generated by the Jimmer DTO language, as its type when defining user-prop")
                return
            }
            ...
            if (o.questionMark == null && dto notModifiedBy Modifier.Specification && StandardType[type] == null) {
                o.error("Type `${o.text}` is not null and its default value cannot be determined")
            }
        }
            ...
            when (configName) {
                ...
                PropConfigName.Recursion.text -> {
                    ...
                    if (qualifiedName == null) {
                        ...
                        val target = qualifiedName.target
                        if (target is Resolution.Target.Type) {
                            process(target.type) { isInheritorOrSelf(JimmerTypes.RecursionStrategy) }?.let {
                                if (!it) {
                                    val className = process(target.type) { className() } ?: target.type.missing("className")
                                    qualifiedName.error("'${className.fqName}' is not a subtype of '${JimmerTypes.RecursionStrategy.name}'")
                                    return
                                }
                            }
                            ...
                            val strategyEntity = process(target.type) {
                                typeArgumentFor(JimmerTypes.RecursionStrategy)
                            }
                ...
                PropConfigName.FetchType.text -> {
                    ...
                    if (fetchType != null) {
                        ...
                        val referenceFetchType = JimmerTypes.ReferenceFetchType.psiClass(element = o) ?: return

...
class DTOCompletionContributor : CompletionContributor() {
    ...
    private fun completeAnnotationParamValue() {
        complete(
            { parameters, result ->
                ...
                val paramType =
                    annotationClass?.methods?.find { it.name == param }?.returnType?.extract ?: return@complete
    ...
    private fun complete(
        provider: (CompletionParameters, CompletionResultSet) -> Unit,
        place: ElementPattern<PsiElement>
    ) {
        extend(
    ...
    private fun List<PsiClass>.lookUp(
        needImport: Boolean = true,
        ...
        LookupElementBuilder.create(qualifiedName, name)
            ...
            .withInsertHandler { context, _ ->
                if (needImport) {
                    ...
                    val annotationName =
                        file.findElementAt(context.startOffset)?.parent?.parentUnSure<DTOQualifiedName>()
